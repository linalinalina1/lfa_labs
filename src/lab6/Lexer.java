...
public class Lexer {
    ...
    private Token number() {
        ...
        if (!TokenType.NUMBER.getPattern().matcher(lexeme).matches()) {
            throw error("Invalid number format");
        }
    ...
    private Token identifierOrKeyword() {
        ...
        return switch (word) {
            ...
            default -> {
                if (!TokenType.IDENTIFIER.getPattern().matcher(word).matches()) {
                    throw error("Invalid identifier");
                }
                yield new Token(TokenType.IDENTIFIER, word, startPos, startLine, startColumn);
            }
        };
    }
